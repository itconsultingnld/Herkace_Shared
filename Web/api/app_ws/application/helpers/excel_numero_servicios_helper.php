...
function excel_reporte_numero_servicios($datos)
{
    $CI = &get_instance();
    $CI->load->database();
    ...
    $queryNums = $CI->db->query('select * from vw_numero_servicios where num_orden in (' . $datos['num_orden'] . ')');

    header('Content-disposition: attachment; filename="' . XLSXWriter::sanitize_filename('Número de servicios.xlsx') . '"');
    ...
    $nombre_hoja = 'Número de servicios';
    ...
    for ($i = 2; $i <= 8; $i++) {
        ...
        if ($i == 2) {
            array_push($styles_fondo, $style_color);
        } else {
            array_push($styles_fondo, $style);
        }
    ...
    $writer->writeSheetHeader($nombre_hoja, $header, ['widths' => [11, 38, 8, 8, 8, 8, 8, 8, 8], $style_h, $style_h, $style_h, $style_h, $style_h, $style_h, $style_h_c, $style_h_c, $style_h_c]);
    ...
    foreach ($queryNums->result() as $row) {
        ...
        if ($cliente != $cliente_ant || $cliente_ant == '') {
            if ($cliente_ant != 0) {
                $array[2] = $total_servicio;
                $totales[0] = $totales[0] + $total_servicio;
                $writer->writeSheetRow($nombre_hoja, $array, $styles);
            }
            $cliente_ant = $cliente;
            $total_servicio = 0;
            $array[0] = $row->num_orden;
            $array[1] = $row->nombre_cliente;
            $array[2] = '0';
            $array[3] = '0';
            $array[4] = '0';
            $array[5] = '0';
    ...
    $writer->writeSheetRow($nombre_hoja, $array, $styles_totales);
