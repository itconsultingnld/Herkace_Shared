...
class MarcasVehiculos extends REST_Controller
{
    public function __construct()
    {
        ...
        $this->load->database();
        $this->load->model("MarcasVehiculos_model");
    }

    public function listado_get()
    {
        $respuesta = $this->MarcasVehiculos_model->listado($this->get());
    ...
    public function agregar_post()
    {
        ...
        if ($this->form_validation->run('marcas_vehiculos_agregar_post')) {
            $respuesta = $this->MarcasVehiculos_model->agregar($datos);
            if ($respuesta["error"] === false) {
    ...
    public function modificar_put()
    {
        $datos = $this->put();
        $this->form_validation->set_data($datos);
        if ($this->form_validation->run('marcas_vehiculos_modificar_put')) {
            $respuesta = $this->MarcasVehiculos_model->modificar($datos);
            if ($respuesta["error"] === false) {
                $this->response($respuesta, 200);
            } else {
                $this->response($respuesta, 400);
    ...
    {
        $respuesta = $this->MarcasVehiculos_model->desactivar($this->put());
        if ($respuesta["error"] === false) {
            $this->response($respuesta, 200);
        } else {
            $this->response($respuesta, 400);
    ...
    {
        $this->load->helper('excel_marcas_vehiculos');
        excel_marcas_vehiculos();
    }
}
