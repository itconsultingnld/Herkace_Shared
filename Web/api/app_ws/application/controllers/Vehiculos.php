...
class Vehiculos extends REST_Controller
{
    ...
    {
        $respuesta = $this->Vehiculos_model->listado();
    ...
    {
        $respuesta = $this->Vehiculos_model->listado_vw();
        if ($respuesta["error"] === false) {
            $this->response($respuesta['registros'], 200);
        } else {
            $this->response($respuesta['registros'], 400);
    ...
    {
        $vehiculo_id = (int) $this->uri->segment(3);
        $respuesta = $this->Vehiculos_model->obtener_por_id($vehiculo_id);
        if ($respuesta["error"] === false) {
    ...
    public function agregar_post()
    {
        ...
        if ($this->form_validation->run('vehiculos_agregar_post')) {
            $respuesta = $this->Vehiculos_model->agregar($datos);
            if ($respuesta["error"] === false) {
    ...
    public function modificar_put()
    {
        $datos = $this->put();
        $this->form_validation->set_data($datos);
        if ($this->form_validation->run('vehiculos_modificar_put')) {
            $respuesta = $this->Vehiculos_model->modificar($datos);
            if ($respuesta["error"] === false) {
                $this->response($respuesta, 200);
            } else {
                $this->response($respuesta, 400);
    ...
    {
        $respuesta = $this->Vehiculos_model->desactivar($this->put());
        if ($respuesta["error"] === false) {
            $this->response($respuesta, 200);
        } else {
            $this->response($respuesta, 400);
    ...
    {
        $this->load->helper('excel_vehiculos');
        excel_vehiculos();
    }
}
